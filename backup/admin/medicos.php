...
if (isset($_GET['delete'])) {
  ...
  $db->query("DELETE FROM doctores WHERE id = '$delete_id'");
          ...
          <div class="content-body">
            ...
            <div class="container-fluid">
                <div class="form-head d-flex mb-sm-4 mb-3">
                    <div class="mr-auto">
                        <h2 class="text-black font-w600">Medicos </h2>
                        <p class="mb-0">Lista de Medicos da clinica</p>
                    </div>
                </div>
                ...
                ***********************************-->
                ...
                <?php
  $sql = "SELECT * FROM doctores WHERE permissions = 'medico' ORDER BY full_name";
  $uresults = $db->query($sql);
if (isset($_GET['add']) || isset($_GET['edit']) ) {
  $permitionsQuery=$db->query("SELECT * FROM doctores ORDER BY permissions");
...
if (isset($_GET['edit'])) {
   ...
   $usersresults = $db->query("SELECT * FROM doctores WHERE id = '$edit_id'");
  ...
  if (!empty($errors)) {
   echo display_errors($errors);
    ...
    $hashed =  password_hash($password, PASSWORD_DEFAULT);
      $insertSql = "INSERT INTO doctores (`full_name`, `email`, `address`, `position_clinic`, `cellphone`, `about`, `especialidade`, `password`, `permissions`) VALUES ('$full_name', '$email', '$address', '$position_clinic', '$cellphone', '$about', '$especialidade', '$hashed', '$permissions')";
      $_SESSION['success_flash'] = "Usuario adicionado com sucesso";
      
   if (isset($_GET['edit'])){
         $insertSql = "UPDATE doctores SET `full_name`='$full_name', `email` ='$email', `address` ='$address', `position_clinic` ='$position_clinic', `cellphone` ='$cellphone', `about` ='$about', `especialidade` ='$especialidade', `password`='$hashed', `last_login`='$last_login', `permissions`='$permissions' WHERE id='$edit_id'";
         $_SESSION['success_flash'] = "Usuario atualizado com sucesso";
   }
   
     $db -> query($insertSql);
echo "<script>alert('Salvo'); window.location = './medicos.php';</script>";
  }
}
  ...
  ?>
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <div class="basic-form">
                                    <form action="medicos.php?<?=((isset($_GET['edit']))?'edit='.$edit_id:'add=1')?>" method="POST">
                                    <form>
                                        ...
                                        <div class="form-row">
                                            ...
                                            <div class="form-group col-md-6">
                                                ...
                                                <select class="form-control default-select" id="permissions" name="permissions">
                                                     ...
                                                     <option value="medico"<?=(($permissions== 'medico' )?' selected':'');?>>Medico</option>
                                                </select>
                                            </div>
                                            <div class="form-group col-md-6">
                                        ...
                                        <a href="medicos.php" class="btn btn-primary mr-3"> Voltar</a>
                                    </form>
                                </div>
                            </div>
                        </div>
                    ...
                    <div>
                        <a href="medicos.php?add=1" class="btn btn-primary mr-3" id="add-product-btn">adicionar novo Usuario</a>
                    </div>
                </div>
                    <div class="row">
                    <div class="col-12">
                                        ...
                                        <td>
                                            <div class="dropdown ml-auto text-right">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right">
                                                    <a class="dropdown-item" href="medico_details.php?details=<?=$user['id'];?>">Ver Detalhes</a>
                                                    <a class="dropdown-item" href="medicos.php?edit=<?=$user['id'];?>">Editar</a>
                                                    <?php
                                                    if ($user['id']!= $user_data['id'] ):?>
                                                    <a class="dropdown-item" href="medicos.php?delete=<?=$user['id'];?>">Apagar</a>
                                                    <?php endif;?>
                                                </div>
                                            </div>
                                        </td>                                               

          ...
          <div class="content-body">
			...
			<div class="container-fluid">
        		...
        		***********************************-->
        		...
        		<?php
  $sql = "SELECT * FROM users WHERE permissions = 'admin,editor' ORDER BY full_name";
...
if (isset($_GET['edit'])) {
   ...
   $department = ((isset( $_POST['department']) && $_POST['department'] != '')?sanitize($_POST['department']):$usersquery['department']);
  ...
  if (!empty($errors)) {
   echo display_errors($errors);
    ...
    $hashed =  password_hash($password, PASSWORD_DEFAULT);
      $insertSql = "INSERT INTO users (`full_name`, `email`, `address`, `position_clinic`, `cellphone`, `about`, `department`, `password`, `permissions`) VALUES ('$full_name', '$email', '$address', '$position_clinic', '$cellphone', '$about', '$department', '$hashed', '$permissions')";
      $_SESSION['success_flash'] = "Usuario adicionado com sucesso";
      
   if (isset($_GET['edit'])){
         $insertSql = "UPDATE users SET `full_name`='$full_name', `email` ='$email', `address` ='$address', `position_clinic` ='$position_clinic', `cellphone` ='$cellphone', `about` ='$about', `department` ='$department', `password`='$hashed', `last_login`='$last_login', `permissions`='$permissions' WHERE id='$edit_id'";
         $_SESSION['success_flash'] = "Usuario atualizado com sucesso";
   }
   
     $db -> query($insertSql);
echo "<script>alert('Salvo'); window.location = './admin_user.php';</script>";
  }
}
  ...
  ?>
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <div class="basic-form">
                                    <form action="admin_user.php?<?=((isset($_GET['edit']))?'edit='.$edit_id:'add=1')?>" method="POST">
                                    <form>
                                        ...
                                        <a href="admin_user.php" class="btn btn-primary mr-3"> Voltar</a>
                                    </form>
                                </div>
                            </div>
                        </div>
                    ...
                    <div>
                        <a href="admin_user.php?add=1" class="btn btn-primary mr-3" id="add-product-btn">adicionar novo Usuario</a>
                    </div>
                </div>
					<div class="row">
					<div class="col-12">
										...
										<td>
											<div class="dropdown ml-auto text-right">
												...
												<div class="dropdown-menu dropdown-menu-right">
													<a class="dropdown-item" href="user_details.php?details=<?=$user['id'];?>">Ver Detalhes</a>
													<a class="dropdown-item" href="admin_user.php?edit=<?=$user['id'];?>">Editar</a>
                                                    <?php
                                                    if ($user['id']!= $user_data['id'] ):?>
													<a class="dropdown-item" href="admin_user.php?delete=<?=$user['id'];?>">Apagar</a>
                                                    <?php endif;?>
												</div>
											</div>
										</td>												
